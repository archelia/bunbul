...
if(isset($_POST["submit"])){
	$description = htmlspecialchars($_POST['peymentdescription']);	
	$howtopay = htmlspecialchars($_POST['howtopay']);	
	
	// if there's posting Edit	
	if($_POST['submit']=="EDIT"){	
		$id = $_POST['id'];
		$sql = "UPDATE paymentmethod ";
		$sql .= "SET method_title='$_POST[paymentmethodtitle]' ";
		$sql .= ",description='$description' ";
		$sql .= ",howto='$howtopay' ";
		$sql .= ", date_edited=now() ";
		$sql .= ", user_edit='$_SESSION[viouser]' ";
		$sql .= "WHERE id_paymentmethod='$id'";
	}
	else{
		// looking for id
		$sql = "SELECT id_paymentmethod FROM paymentmethod ORDER BY id_paymentmethod DESC limit 1";
		$qr = mysql_query($sql);
		...
		$sql .= "VALUES ('', '$_POST[paymentmethodtitle]', '$description', '$howtopay', 
	...
	{
		$success = 1;
		$message = 'Payment Method saved succesfully.';			
	}
	else
	{	
		$success = 0;	
	...
	$target_dir = "../../source/paymentmethod/";
	for ($i = 0; $i < $max; $i++) {
		$filename = "file".($i+1);
